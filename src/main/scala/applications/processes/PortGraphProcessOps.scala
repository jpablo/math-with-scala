...
given PortGraphProcessDSL: ProcessDSL[Graph]:
...
given PortGraphProcessOps: ProcessDSLOps[[A, B] =>> PortGraph[String]]:

  def id[A: TypeName]: Graph[A, A] =
    PortGraph.identity[String]("id", List(tpeName[A]))

  // @alpha("andThen")
  extension [A, B, C](f: Graph[A, B])
    def >>> (g: Graph[B, C]): Graph[A, C] =
      f andThen g
...
object triangle_equations {
  ...
  def process[~>[_, _]](using DSL: ProcessDSLOps[~>]) = {
    ...
    (assocR[X, I, Y] >>> (id[X] ++ snd[I, Y])) <-> fst[X, I] ++ id[Y]
  }
...
object pentagon_equations {
  ...
  def process[~>[_, _]](using DSL: ProcessDSLOps[~>]) = {
    ...
    (assocR[W, X, Y] ++ id[Z]) >>> assocR[W, X * Y, Z] >>> (id[W] ++ assocR[X, Y, Z]) <-> (assocR[W * X, Y, Z] >>> assocR[W, X, Y * Z])
