...
object NaturalTransformation:
  // Helper constructor
  def apply[F[_], G[_], C[_, _]: Category, D[_, _]: Category](
      source: (C --> D)[F],
      target: (C --> D)[G],
      nat: [X] => () => D[F[X], G[X]]
  ): (F ==> G)[C, D] =
  ...
  def identity[F[_], C[_, _]: Category, D[_, _]: Category](
      f: (C --> D)[F]
  ): (F ==> F)[C, D] =
...
extension [H[_], F[_], G[_], Cp[_, _]: Category, C[_, _]: Category, D[
    _,
    _
]: Category](nat: (F ==> G)[C, D])
...
trait NaturalIsomorphism[F[_], G[_], C[_, _]: Category, D[_, _]: Category as D]
...
/** A Natural Isomorphism between the functors F and G.
  *
  * F <==> G
  */
type <===>[F[_], G[_]] =
