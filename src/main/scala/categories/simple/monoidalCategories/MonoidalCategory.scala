...
trait MonoidalCategory[C[_, _]: Category as cat]:
  export cat.*
  type I
  type ⨂[_, _]
  ...
  def a[X, Y, Z]: ((X ⨂ Y) ⨂ Z) ~> (X ⨂ (Y ⨂ Z)) =
    associator.iso[(X, Y, Z)].from
  ...
  extension [A1, A2, B1, B2](f: A1 ~> A2)
    def ⨂(g: B1 ~> B2): (A1 ⨂ B1) ~> (A2 ⨂ B2) =
  ...
  def pentagonEquation[W, X, Y, Z] =
    (a[W, X, Y] ⨂ id[Z]) >>> a[W, X ⨂ Y, Z] >>> (id[W] ⨂ a[X, Y, Z]) <-> (a[
      W ⨂ X,
      Y,
      Z
    ] >>> a[W, X, Y ⨂ Z])
